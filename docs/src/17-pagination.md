...
- The pagination is very simple, here are some output when requesting posts
- You can also disable pagination by adding `use DisablePagination` (add trait to class) to the Post Controller and also just include the trait at the top of the file: 
...
```json
{
  ...
  "links": {
    "first": "http://orion.devserv.me/api/posts?page=1",
    "last": "http://orion.devserv.me/api/posts?page=1",
    "prev": null,
    "next": null
  },
  "meta": {
    ...
    "links": [
      ...
      {
        "url": "http://orion.devserv.me/api/posts?page=1",
        "label": "1",
        "active": true
      },
      {
    ...
    "path": "http://orion.devserv.me/api/posts",
    "per_page": 15,
    "to": 15,
    "total": 15
  }
...
- You will notice that in the `links` and `meta` sections the link are supplied for going to the next and prev pages etc
